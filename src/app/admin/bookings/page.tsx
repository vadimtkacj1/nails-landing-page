...
export default function BookingsPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (!isValidBookingDateString(newDate) || !isBookingDateNotInPast(newDate)) {
      setCreateError('נא לבחור תאריך היום או בעתיד')
      return
    }
    setCreateBusy(true)
    try {
      ...
      if (!res.ok) {
        setCreateError(typeof data.error === 'string' ? data.error : 'שגיאה בשמירה')
        return
      }
      if (data.booking) {
  ...
  const handleSaveEdit = async () => {
    ...
    if (
      ...
      setEditError('תאריך חייב להיות היום או בעתיד (או להשאיר את התאריך הקיים)')
      return
    }
    setSaveBusy(true)
    try {
      ...
      const data = (await res.json().catch(() => ({}))) as { booking?: Booking; error?: string }
      if (!res.ok) {
        setEditError(typeof data.error === 'string' ? data.error : 'שגיאה בשמירה')
        return
      }
      if (data.booking) {
  ...
  const handleDelete = async (id: string) => {
    ...
    const message = detail
      ...
      : 'למחוק את ההזמנה? פעולה זו לא ניתנת לביטול.'
  ...
  return (
    <div>
      <AdminHeader title="ניהול תורים" subtitle={loading ? undefined : `${bookings.length} הזמנות`} />
      ...
      <div className="p-4 md:p-8">
        ...
        >
          <h2 className="text-[15px] font-bold text-[#111827]">תור חדש</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div>
              <label className="block text-[12px] text-[#6b7280] mb-1">שם</label>
              <input
                dir="auto"
                autoComplete="name"
                className={inputName}
            ...
            <div>
              <label className="block text-[12px] text-[#6b7280] mb-1">טלפון</label>
              <input
                dir="ltr"
                autoComplete="tel"
                className={inputLtr}
            ...
            <div>
              <label className="block text-[12px] text-[#6b7280] mb-1">תאריך</label>
              <input
                type="date"
                dir="ltr"
                min={localISODate()}
            ...
            <div>
              <label className="block text-[12px] text-[#6b7280] mb-1">שעה</label>
        ...
        <div className="flex justify-between items-center mb-5 gap-3 flex-wrap">
          <input
            ...
            className="px-4 py-2.5 border border-[#d9d9d9] rounded-lg text-[14px] focus:outline-none focus:ring-2 focus:ring-[#1B1BB3]/20 focus:border-[#1B1BB3] bg-white transition-all text-right w-full sm:w-72"
          />
          <button
            type="button"
        ...
        {!loading && filtered.length === 0 && (
          <div className="bg-white rounded-xl border border-gray-100 py-16 text-center text-[#9ca3af] text-[14px]">
            {search ? 'לא נמצאו תוצאות' : 'אין הזמנות עדיין'}
          </div>
        )}
              ...
              {editingId === booking.id ? (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    >
                      <X size={16} className="inline" /> ביטול
                    </button>
                    <button
                      type="button"
                      ...
                      style={{ background: gradient }}
                    >
                      <Check size={16} className="inline" /> שמור
                    </button>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <div className="text-[11px] text-[#9ca3af]">
                    נרשם:{' '}
                    <LtrField className="!w-auto">{formatCreatedAt(booking.createdAt)}</LtrField>
                  </div>
                </>
              )}
            </div>

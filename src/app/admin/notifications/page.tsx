...
export default function AdminNotificationsPage() {
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setMessage({ type: 'err', text: data.error || 'שגיאה' })
        return
      }
      setSettings(data)
      setMessage({ type: 'ok', text: 'נשמר' })
  ...
  return (
    <div className="min-w-0 max-w-full">
      ...
      <div className="p-4 md:p-8 max-w-[480px]">
        <h1 className="text-[18px] font-semibold text-[#0f172a] mb-5">טלגרם</h1>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-5">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="text-[14px] font-medium text-[#334155]">פעיל</span>
          </label>

          <div className="space-y-2">
            <span className="text-[13px] font-medium text-[#475569]">אופן שליחה</span>
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-[14px] text-[#334155]">פעם ביום בשעה קבועה</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
              ...
              />
              <span className="text-[14px] text-[#334155]">מיד אחרי כל הזמנה חדשה</span>
            </label>
          </div>

          {settings.mode === 'scheduled' && (
            <div>
              <label className="block text-[13px] font-medium text-[#475569] mb-2">שעה</label>
              <input
                type="time"
